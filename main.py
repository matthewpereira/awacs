...
def get_prs(repo, pivotal_members, show_pivots):
    ...
    for pr in repo.get_pulls('all'):
        ...
        if is_pivot(pr.user.login, pivotal_members) and not show_pivots:
            continue

        created = pr.created_at.replace(hour=0, minute=0, second=0, microsecond=0) # squash it down to the date
...
def is_pivot(login, pivotal_member):
    for member in pivotal_member:
        if login == member.login:
            return True
        if login in ['deniseyu', 'xtreme-james-cooper', 'mhuangpivotal', 'julia-pu']:
            return True

    print("Non-pivot: {}".format(login))
...
if __name__ == "__main__":
    ...
    for repo in repos:
        ...
        array_dates, array_occurances, pr_counts = get_prs(repo, pivotal_members, cli_arguments.show_pivots)
